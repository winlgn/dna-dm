...
/**
 * 通过非线性映射对样本空间进行升维后构成线性可分问题，找出最优分隔平面MMH.
 *
 * @author LiuGuining
 */
public class SVM {
...
//                throw new UnsupportedOperationException("Not supported yet.");
    ...
    public static double[][] svm_get_weight(Model model) {
        int i;
        int nr_class = model.nr_class;
        int num = model.SV[0].length;


        double[][] weight = new double[nr_class * (nr_class - 1) / 2][num];
        ...
        for (i = 0; i < nr_class; i++) {
            for (int j = i + 1; j < nr_class; j++) {
                ...
                for (k = 0; k < ci; k++) {
                    for (p = 0; p < num; p++) {
                        weight[q][p] += coef1[si + k] * model.SV[si + k][p].value;
                    }
                }

                for (k = 0; k < cj; k++) {
                    for (p = 0; p < num; p++) {
                        weight[q][p] += coef2[sj + k] * model.SV[sj + k][p].value;
                    }
                }

                for (p = 0; p < num; p++) {
                    weight[q][p] = Math.abs(weight[q][p]);
                }
                q++;
            }
        }
